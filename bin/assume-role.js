    ...
    function patch(fs5) {
      ...
      function patchLchmod(fs6) {
        fs6.lchmod = function(path3, mode, callback) {
          fs6.open(path3, constants.O_WRONLY | constants.O_SYMLINK, mode, function(err, fd) {
            if (err) {
              if (callback)
                callback(err);
              return;
  ...
  "node_modules/fs-extra/lib/remove/rimraf.js"(exports, module2) {
    ...
    function rmkids(p, options, cb) {
      ...
      options.readdir(p, (er, files) => {
        ...
        files.forEach((f3) => {
          rimraf(path3.join(p, f3), options, (er2) => {
            if (errState) {
              return;
            }
            if (er2)
    ...
    function rmkidsSync(p, options) {
      assert(p);
      assert(options);
      options.readdirSync(p).forEach((f3) => rimrafSync(path3.join(p, f3), options));
      if (isWindows) {
...
var require_remove = __commonJS({
  "node_modules/fs-extra/lib/remove/index.js"(exports, module2) {
    ...
    function remove(path3, callback) {
      if (fs5.rm)
        return fs5.rm(path3, { recursive: true, force: true }, callback);
      rimraf(path3, callback);
    }
    function removeSync(path3) {
      if (fs5.rmSync)
        return fs5.rmSync(path3, { recursive: true, force: true });
      rimraf.sync(path3);
    }
    module2.exports = {
      remove: u(remove),
  ...
  "node_modules/micromatch/index.js"(exports, module2) {
    ...
    var micromatch = (list, patterns, options) => {
      ...
      for (let i2 = 0; i2 < patterns.length; i2++) {
        let isMatch = picomatch(String(patterns[i2]), __spreadProps(__spreadValues({}, options), { onResult }), true);
...
var require_path_type = __commonJS({
  "node_modules/path-type/index.js"(exports) {
    ...
    async function isType(fsStatType, statsMethodName, filePath) {
      ...
      try {
        const stats = await promisify3(fs5[fsStatType])(filePath);
  ...
  "node_modules/ignore/index.js"(exports, module2) {
    ...
    var Ignore = class {
      ...
      _test(originalPath, cache, checkUnignored, slices) {
        ...
        checkPath(path3, originalPath, this._allowRelativePaths ? RETURN_FALSE : throwError);
...
var require_util_map_includes = __commonJS({
  "node_modules/yaml/dist/compose/util-map-includes.js"(exports) {
    ...
    function mapIncludes(ctx, items, search) {
      ...
      if (uniqueKeys === false)
        return false;
      const isEqual2 = typeof uniqueKeys === "function" ? uniqueKeys : (a, b) => a === b || Node.isScalar(a) && Node.isScalar(b) && a.value === b.value && !(a.value === "<<" && ctx.schema.merge);
  ...
  "node_modules/minimist/index.js"(exports, module2) {
    module2.exports = function(args, opts) {
      ...
      function setArg(key2, val, arg2) {
        ...
        setKey(argv3, key2.split("."), value2);
        (aliases[key2] || []).forEach(function(x2) {
          setKey(argv3, x2.split("."), value2);
        });
      }
      ...
      for (var i2 = 0; i2 < args.length; i2++) {
        ...
        if (/^--.+=/.test(arg)) {
          ...
          if (!flags.unknownFn || flags.unknownFn(arg) !== false) {
            argv3._.push(flags.strings["_"] || !isNumber(arg) ? arg : Number(arg));
          }
          if (opts.stopEarly) {
            argv3._.push.apply(argv3._, args.slice(i2 + 1));
            break;
          }
        }
      }
      Object.keys(defaults).forEach(function(key2) {
        if (!hasKey(argv3, key2.split("."))) {
          setKey(argv3, key2.split("."), defaults[key2]);
          (aliases[key2] || []).forEach(function(x2) {
            setKey(argv3, x2.split("."), defaults[key2]);
          });
        }
      });
      if (opts["--"]) {
...
var checkCwdOption = (options) => {
  ...
  let stat2;
  try {
    stat2 = import_node_fs2.default.statSync(options.cwd);
  } catch {
    return;
  }
...
function hasFlag(flag, argv3 = import_node_process2.default.argv) {
...
var argv2 = (0, import_minimist.default)(process.argv.slice(2));
var globby2 = Object.assign(function globby3(...args) {
  return globby(...args);
}, globby_exports);
var glob = globby2;
var psTree = (0, import_util.promisify)(import_ps_tree.default);
...
function $2(pieces, ...args) {
  ...
  while (i2 < args.length) {
    ...
    if (Array.isArray(args[i2])) {
      s2 = args[i2].map((x2) => $2.quote(substitute(x2))).join(" ");
    } else {
      s2 = $2.quote(substitute(args[i2]));
    }
    cmd += s2 + pieces[++i2];
  }
  let resolve, reject;
  let promise = new ProcessPromise((...args2) => [resolve, reject] = args2);
  promise._run = () => {
    if (promise.child)
      return;
    if (promise._prerun)
...
$2.quote = quote;
$2.spawn = import_child_process.spawn;
$2.verbose = true;
$2.prefix = "";
try {
  $2.shell = import_which.default.sync("bash");
  $2.prefix = "set -euo pipefail;";
} catch (e2) {
}
function cd(path3) {
  if ($2.verbose)
    console.log("$", colorize(`cd ${path3}`));
  process.chdir(path3);
}
async function question2(query, options) {
  let completer = void 0;
  if (Array.isArray(options == null ? void 0 : options.choices)) {
...
function printCmd(cmd) {
  if (/\n/.test(cmd)) {
    console.log(cmd.split("\n").map((line, i2) => (i2 === 0 ? "$" : ">") + " " + colorize(line)).join("\n"));
  } else {
...
function colorize(cmd) {
  return cmd.replace(/^[\w_.-]+(\s|$)/, (substr) => {
    return source_default.greenBright(substr);
  });
}
...
// node_modules/zx/src/globals.mjs
registerGlobals();
...
var assumeRole = async (_roleArn) => {
  ...
  const accessKeyId = (0, import_get.default)(json, "Credentials.AccessKeyId");
  const secretAccessKey = (0, import_get.default)(json, "Credentials.SecretAccessKey");
  const sessionToken = (0, import_get.default)(json, "Credentials.SessionToken");
  const expiration = (0, import_get.default)(json, "Credentials.Expiration");
...
var getUserId = async () => {
  ...
  const userId = (0, import_get.default)(json, "UserId");
  log("User id: ", userId);
  return userId;
};
var createSecureSession = async (mfaArn, token) => {
...
var checkAwsCliVersion = async () => {
  ...
  const version = (0, import_toInteger.default)(matches.groups.version);
  if (version !== 2) {
